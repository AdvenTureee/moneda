...
**Phone (E.164):** `NULL` na criação (Supabase Auth pode autenticar por email/OAuth sem phone), preenchido no onboarding ou em Perfil > WhatsApp. É opcional para uso geral do app. O lançamento pelo WhatsApp fica indisponível até o fluxo próprio de confirmação por código via WhatsApp Business do Moneda estar pronto. `UNIQUE WHERE phone IS NOT NULL` (partial unique index — Postgres permite múltiplos `NULL`s, mas dois usuários nunca podem reivindicar o mesmo número).
